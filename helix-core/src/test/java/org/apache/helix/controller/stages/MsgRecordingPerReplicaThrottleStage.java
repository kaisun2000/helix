...
public class MsgRecordingPerReplicaThrottleStage extends PerReplicaThrottleStage {
  ...
  protected void applyThrottling(Resource resource,
      ...
      Map<Partition, Map<String, String>> bestPossibleMap, IdealState idealState,
      ResourceControllerDataProvider cache, boolean onlyDownwardLoadBalance, List<Message> messages,
      Map<Message, Partition> messagePartitionMap, Set<Message> throttledMessages,
      StateTransitionThrottleConfig.RebalanceType rebalanceType) {
    Set<Message> middleThrottledMessages = new HashSet<>();
    super.applyThrottling(resource, throttleController, currentStateMap, bestPossibleMap, idealState,
            cache, onlyDownwardLoadBalance, messages, messagePartitionMap, middleThrottledMessages,
            rebalanceType);
